...
int main( int argc, char *argv[] ) {
    ...
    float *dev_u, *dev_uhelp, *residual_cuda, *aux_cuda;
    ...
    cudaMalloc(&residual_cuda,sizeof(float)*Grid_Dim*Grid_Dim );
    aux_cuda = (float*)malloc(Grid_Dim*Grid_Dim*sizeof(float));
    ...
    iter = 0;
    while(1) {
        gpu_Heat<<<Grid,Block,Block_Dim*Block_Dim*sizeof(float)>>>(dev_u, dev_uhelp, np, residual_cuda);
        ...
        //...
	cudaMemcpy(aux_cuda,residual_cuda,sizeof(float)*Grid_Dim*Grid_Dim,cudaMemcpyDeviceToHost);
	
	residual = 0.0;
	cudaThreadSynchronize(); //aqui si que hay que esperar, para que se acabe la copia
    ...
    fprintf(stdout, "(%3.3f GFlop => %6.2f MFlop/s)\n", 
	    flop/1000000000.0,
	    flop/elapsed_time_ms/1000);
    fprintf(stdout, "Convergence to residual=%f: %d iterations", residual, iter);
